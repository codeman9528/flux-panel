...
public class WebSocketServer extends TextWebSocketHandler {
    ...
    // 每个节点一把锁，串行化同一节点的并发连接，保证"接管判定 + 会话覆盖 + 令牌写入"原子
    private static final ConcurrentHashMap<Long, Object> nodeConnectLocks = new ConcurrentHashMap<>();
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        try {
            ...
            if (!Objects.equals(type, "1")) {
                ...
                // 同一节点的并发连接串行处理，避免接管判定与写入竞态
                synchronized (nodeConnectLocks.computeIfAbsent(nodeId, k -> new Object())) {
                ...
                if (incomingInstallId != null && incomingInstallId > 0 && ownerCheck != null && ownerCheck.getInstallId() != null
                ...
                if (node != null) {
                    ...
                    if (incomingInstallId != null && incomingInstallId > 0) {
                ...
                } // end synchronized(nodeConnectLocks)
            }
