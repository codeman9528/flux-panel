...
export default function ForwardPage() {
  ...
  // 只保留分类(可折叠)模式；平铺模式的分支保留为死代码不再触发
  const [viewMode] = useState<'grouped' | 'direct'>('grouped');
  ...
  // 全部收起 / 全部展开（分类模式下所有隧道的折叠开关）
  const toggleAllTunnels = () => {
    setExpandedTunnelKeys(prev => {
      if (prev.size > 0) return new Set();
      const keys = new Set<string>();
      forwards.forEach((f: Forward) => {
        if (f.userId != null && f.tunnelId != null) keys.add(`${f.userId}-${f.tunnelId}`);
      });
      return keys;
    });
  };
  ...
  // 右侧浮动菜单项样式：图标在上、文字在下的竖排小按钮
  const menuItemCls = "h-auto min-w-0 flex-col gap-0.5 py-2 px-1 text-xs";
  ...
  return (
      ...
      <div className="px-3 lg:px-6 py-8 lg:pr-24">
        ...
        <div className="sticky top-0 z-30 bg-gray-100 dark:bg-black pt-2 pb-4 flex flex-nowrap items-center gap-1 [&>*]:flex-1 [&>*]:!min-w-0 lg:gap-3 lg:justify-end lg:[&>*]:flex-none">
            ...
            <Button
              size="sm"
              variant={activeSearch ? 'solid' : 'bordered'}
              color="secondary"
            ...
            <Button
              size="sm"
              variant={selectionMode ? 'solid' : 'bordered'}
              color="danger"
        ...
        {/* 右侧浮动竖排菜单：中间偏下、窄条 */}
        <div className="fixed right-3 top-[55%] -translate-y-1/2 scale-[0.8] origin-right z-30 hidden lg:flex flex-col items-stretch gap-1 w-14 max-h-[80vh] overflow-y-auto bg-content1/95 backdrop-blur border border-divider rounded-2xl p-1.5 shadow-lg">
          {/* 单条：新增单条转发 */}
          <Button size="sm" variant="light" className={menuItemCls} onPress={handleAdd} title="新增单条转发">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 3H6a1 1 0 00-1 1v16a1 1 0 001 1h12a1 1 0 001-1V9m-6-6l6 6m-6-6v6h6M9 15h6m-3-3v6" /></svg>
            <span>单条</span>
          </Button>

          {/* 批量：批量导入规则 */}
          <Button size="sm" variant="light" className={menuItemCls} onPress={handleImport} title="批量导入规则">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V5a1 1 0 011-1h10a1 1 0 011 1v10a1 1 0 01-1 1h-2M4 9h10a1 1 0 011 1v10a1 1 0 01-1 1H4a1 1 0 01-1-1V10a1 1 0 011-1z" /></svg>
            <span>批量</span>
          </Button>

          {/* 导出 */}
          <Button size="sm" variant="light" className={menuItemCls} onPress={handleExport} isLoading={exportLoading} title="导出转发规则">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M12 4v12m0-12l-4 4m4-4l4 4" /></svg>
            <span>导出</span>
          </Button>

          <div className="h-px bg-divider mx-1 my-0.5" />

          {/* 启动：批量启动选中规则 */}
          <Button size="sm" variant="light" className={menuItemCls} isDisabled={!hasSelection} isLoading={batchLoading} onPress={handleBatchResume} title="批量启动选中规则">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
            <span>启动</span>
          </Button>

          {/* 暂停：批量暂停选中规则 */}
          <Button size="sm" variant="light" className={menuItemCls} isDisabled={!hasSelection} isLoading={batchLoading} onPress={handleBatchPause} title="批量暂停选中规则">
          ...
          {/* 切换：把选中规则批量迁移到其它隧道 */}
          <Button size="sm" variant="light" className={menuItemCls} isDisabled={!hasSelection} isLoading={batchLoading} onPress={handleBatchMove} title="把选中规则切换到其它隧道">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4M16 17H4m0 0l4 4m-4-4l4-4" /></svg>
            <span>切换</span>
          </Button>

          {/* 删除：批量删除选中规则 */}
          <Button size="sm" variant="light" color="danger" className={menuItemCls} isDisabled={!hasSelection} isLoading={batchLoading} onPress={handleBatchDelete} title="批量删除选中规则">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M10 11v6M14 11v6" /></svg>
            <span>删除</span>
          </Button>
        </div>
                  ...
                  <CardBody className="pt-0">
                    <Accordion
                      ...
                      onSelectionChange={(keys) => {
                        const groupKeys = userGroup.tunnelGroups.map(t => `${userGroup.userId}-${t.tunnelId}`);
                    ...
                    >
                      {userGroup.tunnelGroups.map((tunnelGroup) => (
                        <AccordionItem
                          key={`${userGroup.userId}-${tunnelGroup.tunnelId}`}
          ...
          <ModalContent>
            ...
            <ModalFooter>
              ...
              <Button color="primary" onPress={() => {
                const trimmed = { name: searchForm.name.trim(), tunnelId: searchForm.tunnelId, inPort: searchForm.inPort.trim(), remoteAddr: searchForm.remoteAddr.trim() };
                const has = trimmed.name || trimmed.tunnelId || trimmed.inPort || trimmed.remoteAddr;
                setActiveSearch(has ? trimmed : null);
                setSearchOpen(false);
              }}>搜索</Button>
            </ModalFooter>
          </ModalContent>
